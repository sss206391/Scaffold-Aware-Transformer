...
if __name__ == '__main__':
    ...
    config.predictor_path = "../package/logics_pik3ca_rfr_cv2.pkl"
    
    config.max_epoch = 31
    config.save_period = 2
    config.save_ckpt_fmt = 'pik3ca_logics_e%d.ckpt'
    config.sample_fmt = 'pik3ca_logics_e%d.txt'
    config.memory_fmt ='pik3ca_logics_mem_e%d.csv'
    ...
    config.vc_fmt = 'pik3ca_logics_vc_e%d.smi'  # save valid & canonical smiles
    config.npfps_fmt = 'pik3ca_logics_npfps_e%d.npy'  # save fingerprint in npy
    config.fcvec_fmt ='pik3ca_logics_fcvec_e%d.npy'  # save Frechet ChemNet vectors
    ...
    with open("../../dataset/pik3ca/pik3ca_fold_splits.json", 'r') as f:
        pik3_folds = json.load(f)
    
    # data_npfps = np.load(project_paths['PIK3CA_DATA_FP'])
    # data_fcvecs = np.load(project_paths['PIK3CA_DATA_FCVEC'])
    
    data_npfps = np.load("../../dataset/pik3ca/pik3ca_aff_npfps.npy")
    data_fcvecs = np.load("../../dataset/pik3ca/pik3ca_aff_fcvec.npy")
    ...
    for scale in scales:
        ...
        paths_vc[model_name] = f"pik3ca_logics_vc_e{scale}.smi"
        paths_npfps[model_name] = f"pik3ca_logics_npfps_e{scale}.npy"
        paths_fc_vecs[model_name] = f"pik3ca_logics_fcvec_e{scale}.npy"
    ...
    # loading predictor 
    pred_path = "../package/logics_pik3ca_rfr_cv2.pkl"
    with open(pred_path, 'rb') as f:
        predictor = pickle.load(f)
    predictor
    
    # loading test set actives (tsa)
    affinity_data = pd.read_csv("../../dataset/pik3ca/pik3ca_affinity.csv")
    with open("../../dataset/pik3ca/pik3ca_fold_splits.json", 'r') as f:
        folds = json.load(f)
    ...
    perf_table.to_csv(f'logics_pik3ca_performance.csv')
    perf_table